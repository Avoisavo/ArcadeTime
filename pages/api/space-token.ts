...
import { Connection, Keypair, PublicKey, Commitment, ConnectionConfig } from '@solana/web3.js';
...
let spaceToken: SpaceToken;

try {
  mintAuthority = getMintAuthority();
  spaceToken = new SpaceToken(connection, mintAuthority);
} catch (error: any) {
  console.error('Error initializing Space token:', error);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Check if Space token is properly initialized
    if (!spaceToken) {
      return res.status(500).json({ error: 'Space token not properly initialized. Check server logs for details.' });
    }
    ...
    if (action === 'initialize') {
      if (!mintAddress) {
        ...
        try {
          ...
          // If token already exists, use a default address
          if (error?.message?.includes('already in use') || 
              error?.message?.includes('already exists')) {
            console.log('Token already exists, using default address...');
    ...
    if (action === 'mint') {
      ...
      const signature = await retryWithBackoff(() => 
        spaceToken.mintTokenToUser(new PublicKey(userWallet), amount || 1)
      );
    ...
    console.error('Error in Space token API:', error);
